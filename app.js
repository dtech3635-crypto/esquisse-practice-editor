...
function drawPiece(p){const g=document.querySelector(`[data-grid="${p.floor}"]`);if(!g)return;const t=types.find(x=>x.id===p.type);if(!t)return;const e=document.createElement('div'),totalW=gridW(),totalH=gridH(),measure=`${p.w}×${p.h}m・${p.w*p.h}㎡`;e.className='piece'+(p.id===selectedPiece?' selected':'');e.dataset.pid=p.id;e.style.cssText=`left:${p.x/totalW*100}%;top:${p.y/totalH*100}%;width:${p.w/totalW*100}%;height:${p.h/totalH*100}%;background:${t.color}`;e.innerHTML=`<span>${t.name}<small>${$('#areas').checked?`<br>${measure}`:''}</small></span>`;g.append(e)}
